...
abstract class DescribeTableTestBase extends PaimonSparkTestBase {
  ...
  test("Paimon show: show table extended") {
    ...
    withTable(s"$dbName0.s1") {
      ...
      withDatabase(testDB) {
        ...
        sql(s"USE $testDB")
        withTable("s2", "s3") {
          sql("CREATE TABLE s2 (id INT, pt STRING) PARTITIONED BY (pt)")
          sql("CREATE TABLE s3 (id INT, pt1 STRING, pt2 STRING) PARTITIONED BY (pt1, pt2)")

          sql("INSERT INTO s2 VALUES (1, '2024'), (2, '2024'), (3, '2025'), (4, '2026')")
          sql("""
                |INSERT INTO s3
                |VALUES
          ...
          val showTableExtended1 = sql(s"SHOW TABLE EXTENDED IN $dbName0 LIKE '*'")
          assert(
            showTableExtended1.queryExecution.analyzed
              .isInstanceOf[PaimonShowTablesExtendedCommand])
          ...
          checkAnswer(
            ...
            Row("test", "s1", false))
          val showTableExtended2 = sql(s"SHOW TABLE EXTENDED IN $testDB LIKE '*'")
          assert(
            showTableExtended2.queryExecution.analyzed
              .isInstanceOf[PaimonShowTablesExtendedCommand])
          checkAnswer(
            showTableExtended2
              .select("namespace", "tableName", "isTemporary"),
          ...
          // check table s1
          val res1 = sql(s"SHOW TABLE EXTENDED IN $testDB LIKE 's2'").select("information")
          ...
          val error1 = intercept[Exception] {
            sql(s"SHOW TABLE EXTENDED IN $testDB LIKE 's2' PARTITION(pt='2022')")
          }.getMessage
          assert(error1.contains("PARTITIONS_NOT_FOUND"))

          val error2 = intercept[Exception] {
            sql(s"SHOW TABLE EXTENDED IN $testDB LIKE 's3' PARTITION(pt1='2024')")
          }.getMessage
          assert(error2.contains("Partition spec is invalid"))

          val res2 =
            sql(s"SHOW TABLE EXTENDED IN $testDB LIKE 's3' PARTITION(pt1 = '2024', pt2 = 11)")
          assert(res2.queryExecution.analyzed.isInstanceOf[PaimonShowTablePartitionCommand])
          checkAnswer(
  ...
  test(s"Paimon describe: describe table comment") {
    ...
    comment = "new comment"
    sql(s"ALTER TABLE T SET TBLPROPERTIES ('comment' = '$comment')")
    checkTableCommentEqual("T", comment)
